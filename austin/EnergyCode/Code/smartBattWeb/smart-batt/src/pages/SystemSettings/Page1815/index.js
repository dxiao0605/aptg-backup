...
import { ajaxGetCommandSetup, ajaxUpdCommandSetup } from "./getApi";
...
class Page1815 extends Component {
	...
	handleSubmit = () => {
		...
		return ajaxUpdCommandSetup({query,postData}).then((response) => {
			if (this.ajaxCancel) {
				return new Promise((resolve, reject) => reject(""));
			} //強制結束頁面
	...
	getDefaultSetting = () => {
		const { token, curLanguage, timeZone, company } = this.props;
		const query = { token, curLanguage, timeZone, company};
        ajaxGetCommandSetup({query}).then((response) => {
			if (this.ajaxCancel) {
				return new Promise((resolve, reject) => reject(""));
			} //強制結束頁面
			if (Object.keys(response).length === 0) {
