...
/**
 * Servlet implementation class addCorrectionIR 新增命令任務(校正內阻)
 */
@WebServlet("/addCorrectionIR")
public class addCorrectionIR extends HttpServlet {
	...
	private static final Logger logger = LogManager.getFormatterLogger(addCorrectionIR.class.getName()); 

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public addCorrectionIR() {
		super();
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		logger.debug("addCorrectionIR start");
...
//			String timezone = ObjectUtils.toString(request.getHeader("timezone"));
		...
		logger.debug("addCorrectionIR end");
	...
	private CommandTaskVO parseJson(String json) throws Exception {
		...
		try {
			...
			JSONArray irArr = msg.getJSONArray("IR");						
			String hexConfig = StringUtils.leftPad(Integer.toHexString(irArr.length()*2),2,"0");
			for(int i=0; i<irArr.length(); i++) {
				hexConfig += StringUtils.leftPad(Integer.toHexString(irArr.getInt(i)),4,"0");
			}
			
			commandTaskVO.setConfig(msg.optString("IR"));
