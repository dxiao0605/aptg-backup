...
/**
 * Servlet implementation class addBatteryGroup 新增站台
 */
@WebServlet("/addBatteryGroup")
public class addBatteryGroup extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private static final Logger logger = LogManager.getFormatterLogger(addBatteryGroup.class.getName()); 

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public addBatteryGroup() {
		super();
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		logger.debug("addBatteryGroup start");
				...
				if (!ToolUtil.checkToken(token)) {
					...
					if(batteryGroupVO.isError()) {
						...
						if (list != null && list.size() > 0) {
					    	...
					    	//新增站台
							batteryGroupDAO.addBatteryGroup(batteryGroupVO);
		...
		logger.debug("addBatteryGroup end");
	...
	private BatteryGroupVO parseJson(String json, ResourceBundle resource) throws Exception {
		...
		try {
			...
			if(ToolUtil.isNull(request, "Company")) {
				...
				batteryGroupVO.setDescription(resource.getString("1064")+resource.getString("5008"));//公司必填欄位不能為空
			}else {
				batteryGroupVO.setCompanyCode(request.optString("Company"));
			}
			...
			if(ToolUtil.isNull(request, "Address")) {
				...
				JSONObject address = ToolUtil.getLonLat(request.optString("Address"));//呼叫google API查詢經緯度
				if(address!=null) {
					batteryGroupVO.setLat(address.optString("lat"));
					batteryGroupVO.setLng(address.optString("lng"));
				}
			}
			batteryGroupVO.setDefaultGroup("1");//非預設站台
