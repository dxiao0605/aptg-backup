...
/**
 * Servlet implementation class addCorrectionVolTask 新增命令任務(校正電壓B5)
 */
@WebServlet("/addCorrectionVolTask")
public class addCorrectionVolTask extends HttpServlet {
	...
	private static final Logger logger = LogManager.getFormatterLogger(addCorrectionVolTask.class.getName()); 

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public addCorrectionVolTask() {
		super();
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		logger.debug("addCorrectionVolTask start");
			...
			if (StringUtils.isNotBlank(token) && StringUtils.isNotBlank(req)) {
				...
				if (!ToolUtil.checkToken(token)) {
					...
					if(commandTaskVO.isError()) {
						...
						commandTaskVO.setCategory("2");//電壓
						List<DynaBean> list = commandTaskDAO.getBatteryDetailCount(commandTaskVO);//查詢電壓設定筆數						
		...
		logger.debug("addCorrectionVolTask end");
	...
	private CommandTaskVO parseJson(String json, ResourceBundle resource) throws Exception {
		...
		try {
			...
			//20220307 David 選擇母機時不檢查離線
			
			if(!commandTaskVO.getBatteryId().equals("0"))
			if("4".equals(status)) {
				commandTaskVO.setError(true);
			...
			JSONArray volArr = request.getJSONArray("Vol");						
			String hexConfig = StringUtils.leftPad(Integer.toHexString(volArr.length()*2),2,"0");
			for(int i=0; i<volArr.length(); i++) {
				if(volArr.isNull(i)||StringUtils.isBlank(volArr.optString(i))) {
					commandTaskVO.setError(true);
					commandTaskVO.setCode("24");
					commandTaskVO.setDescription(resource.getString("5008"));//必填欄位不能為空
					return commandTaskVO;
				}else if(volArr.optInt(i)<1000||volArr.optInt(i)>60000) {
					commandTaskVO.setError(true);
					commandTaskVO.setCode("26");
					commandTaskVO.setDescription(resource.getString("5033"));//值域錯誤
					return commandTaskVO;
				}
				hexConfig += StringUtils.leftPad(Integer.toHexString(volArr.optInt(i)),4,"0");
			}
			commandTaskVO.setRecords(volArr.length());
			commandTaskVO.setConfig(request.optString("Vol"));
