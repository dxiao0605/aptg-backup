...
public class NbListDAO extends BaseDAO {
	...
	public void updNBListAllocate(NbListVO nbListVO) throws Exception {
		...
		try {
			 ...
			 *	更換站台，刪除BatteryDetail舊資訊
			 */
			StringBuffer sql7 = new StringBuffer();
			sql7.append(" Delete From BatteryDetail ");
			sql7.append(" where NBID = ? ");
			ps7 = connection.prepareStatement(sql7.toString());

			for(String nbid : nbListVO.getNbList()) {
				ps7.setString(1, nbid);
			}
			ps7.executeBatch();
			/*
			 * edit by Austin (2021/12/28)
	...
	public void updGroupSetup(NbListVO nbListVO) throws Exception {	
		...
		try {
			...
			sql10.append(" Delete From BatteryDetail ");
			sql10.append(" where NBID = ? ");
			ps10 = connection.prepareStatement(sql10.toString());
			ps10.setString(1, nbListVO.getNbId());
			ps10.execute();
			/*
			 * edit by Austin (2021/12/28)
			 * =============== End ===============
	 ...
	 * 查詢通訊板歸屬站台歷史
	 * 
	 * (edit by Austin (2021/12/09): 關聯BatteryGroup取得CompanyCode，處理不可看到不同公司的電池歷史問題
	 * 
	 * @param nbListVO
	 * @return
	 * @throws Exception
	...
	public List<DynaBean> getNBGroupHis(NbListVO nbListVO) throws Exception {
		List<String> parameterList = new ArrayList<String>();
		StringBuffer sql = new StringBuffer();
		sql.append(" select h.*, bg.CompanyCode ");	// edit by Austin (2021/12/09)
		sql.append(" from NBGroupHis h  ");
		sql.append(" , BatteryGroup bg ");	// add by Austin (2021/12/09)
		sql.append(" where 1=1 ");
		sql.append(" and h.GroupInternalID=bg.SeqNo ");	// add by Austin (2021/12/09)
...
//			sql.append(" and bg.CompanyCode = ? ");	// add by Austin (2021/12/09) ***還不確定是否應該加
//			parameterList.add(nbListVO.getCompanyCode());
			...
			if (nbListVO.getCompanyCode()!=null) {	// RCE CompanyCode is null (RCE不加CompanyCode條件，所以故意不設定CompanyCode)
				sql.append(" and bg.CompanyCode = ? ");	// add by Austin (2021/12/09)
				parameterList.add(nbListVO.getCompanyCode());
			}
		}
