...
import FadeMenu from './FadeMenu';//table點點點功能
...
class Table extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            tbody: [],
            row: 10,//目前每頁顯示筆數
            menuList: [CompareWindowns.Authority.model, CompareWindowns.ForwardUser.model, CompareWindowns.Del.model],
        };
    }
    componentDidMount() {
        const { data, perPage, limits } = this.props;
        this.setState({
            row: perPage,
            tbody: [...data]
        });
        const newMenuList = this.state.menuList.map(item => {
            switch (item) {
                case CompareWindowns.Del.model:
                    return (limits?.Button?.RoleDel === 1) ? CompareWindowns.Del.model : '';
    ...
    render() {
        ...
        const { active, addEvent, exportExcel, tableErrMsg, tableHeader, limits } = this.props;
        return (
            <Fragment>
                <div className="col-12 p-0 mb-1">
                    ...
                    <div className="col-12 col-sm-8 d-inline-block text-right p-0" style={{verticalAlign: 'bottom'}}>
                        <Toolbar isAddItem={limits?.Edit === 1 && limits?.Button?.RoleCreate === 1 ? true : false} isExportExcel={true} addEvent={addEvent} exportExcel={exportExcel} />
                    </div>
                </div>
                            ...
                            {tbody?.length > 0 ?
                                tbody.slice(row * (active - 1), row * active)
                                    .map((item, idx) => {
                                        return (
                                            <tr key={item.Seq}>
                                                {tableHeader[0].active && <td>{item.RoleName}</td>}
                                                {tableHeader[1].active && <td>{item.RoleDesc}</td>}
                                                {tableHeader[2].active && <td>{item.RoleDescE}</td>}
                                                {tableHeader[3].active && <td>{item.RoleDescJ}</td>}
                                                {tableHeader[4].active && <td>{item.Count}</td>}
                                                {limits.Edit === 1 && Array.isArray(tableHeader) && tableHeader.length > 0 &&
                                                    <td className="text-center">
                                                        <FadeMenu
                                                            ...
                                                            HandleforwardUserSumbit={this.props.HandleforwardUserSumbit}
                                                            HandleAuthorityClick={this.props.HandleAuthorityClick}
                                                        />
                                                    </td>
                                                }
                                            </tr>
                                ...
                                : <tr className="text-center td_btn-config">
                                    <td colSpan={limits.Edit === 1 ? (tableHeader.length + 1) : tableHeader.length} >{tableErrMsg}</td>
                                </tr>}
                        </tbody>
                    </table>
                </div>
    ...
    onMenuClick = ({ event, data, item }) => {
        const ModelList = Object.values(CompareWindowns).filter(ite => ite.model === item);
        if (Array.isArray(ModelList) && ModelList.length > 0) {
            this.props.getData({ model: ModelList[0].model, data, title: ModelList[0].title });
        }
    }
...
Table.propTypes = {
    ...
    HandleforwardUserSumbit: PropTypes.func,
