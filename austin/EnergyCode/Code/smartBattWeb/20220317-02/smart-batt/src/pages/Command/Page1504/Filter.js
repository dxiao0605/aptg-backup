...
class Filter extends Component {
    ...
    getCommandList = (companyVal) => {
        ...
        return this.ajaxGetCommandList({ companyCode }).then((response) => {
            if (this.ajaxCancel) { return; }//強制結束頁面
            if (Object.keys(response).length === 0) { return; }
            const { code, msg } = response;
            if (code === '00' && msg) {
    ...
    getCommandBattIdList = () => {
        ...
        return this.ajaxGetCommandBattIdList({ groupInternalId, commandId: radioVal }).then((response) => {
            if (this.ajaxCancel) { return; }//強制結束頁面
            if (Object.keys(response).length === 0) { return; }
            const { code, msg } = response;
            if (code === '00' && msg) {
    ...
    getIRSetting = () => {
        ...
        return this.ajaxGetIRSetting({ batteryGroupId }).then((response) => {
            if (this.ajaxCancel) { return; }//強制結束頁面
            if (Object.keys(response).length === 0) { return; }
            const { code, msg } = response;
            if (code === '00' && msg) {
    ...
    getPeriodSetting = () => {
        ...
        if (batteryGroupId.length === 0) { return; }
        return this.ajaxGetPeriodSetting({ batteryGroupId }).then((response) => {
            if (this.ajaxCancel) { return; }//強制結束頁面
            if (Object.keys(response).length === 0) { return; }
            const { code, msg } = response;
            if (code === '00' && msg) {
    ...
    getCorrectionVolg = () => {
        ...
        if (batteryGroupId.length === 0) { return; }
        return this.ajaxGetCorrectionVol({ batteryGroupId }).then((response) => {
            if (this.ajaxCancel) { return; }//強制結束頁面
            if (Object.keys(response).length === 0) { return; }
            const { code, msg } = response;
            if (code === '00' && msg) {
    ...
    getCorrectionIR = () => {
        ...
        if (batteryGroupId.length === 0) { return; }
        return this.ajaxGetCorrectionIR({ batteryGroupId }).then((response) => {
            if (this.ajaxCancel) { return; }//強制結束頁面
            if (Object.keys(response).length === 0) { return; }
            const { code, msg } = response;
            if (code === '00' && msg) {
    ...
    addIRSettingBatch = (data, CommandID) => {
        ...
        return this.ajaxAddIRSettingBatch(postData).then((response) => {
    ...
    addPeriodSettingBatch = (data, CommandID) => {
        ...
        return this.ajaxAddPeriodSettingBatch(postData).then((response) => {
            if (this.ajaxCancel) { return new Promise((resolve, reject) => (reject(''))); }//強制結束頁面
            if (Object.keys(response).length === 0) { return new Promise((resolve, reject) => (reject(''))); }
            const { code, msg } = response;
            const title = '1089';
    ...
    addCorrectionVolTask = (data, CommandID) => {
        ...
        return this.ajaxAddCorrectionVolTask(postData).then((response) => {
            if (this.ajaxCancel) { return new Promise((resolve, reject) => (reject(''))); }//強制結束頁面
            if (Object.keys(response).length === 0) { return new Promise((resolve, reject) => (reject(''))); }
            const { code, msg } = response;
            const title = '1089';
    ...
    addCorrectionIRTask = (data, CommandID) => {
        ...
        return this.ajaxAddCorrectionIRTask(postData).then((response) => {
            if (this.ajaxCancel) { return new Promise((resolve, reject) => (reject(''))); }//強制結束頁面
            if (Object.keys(response).length === 0) { return new Promise((resolve, reject) => (reject(''))); }
            const { code, msg } = response;
            const title = '1089';
    ...
    //國家/地域/站台下拉選單:
    ajaxGetCommandList = ({ companyCode }) => {
        const { token, curLanguage, timeZone, company } = this.props;
        const { radioVal } = this.state;    //commandId
        const url = `${apiUrl}getCommandList?companyCode=${companyCode}&commandId=${radioVal}`;
        return ajax(url, 'GET', token, curLanguage, timeZone, company)
        //國家/地域/站台下拉選單:
        //https://www.gtething.tw/battery/getCommandList?companyCode=1
    }
    //電池組ID 下拉選單(GET):
    ajaxGetCommandBattIdList = ({ groupInternalId, commandId }) => {
        const { token, curLanguage, timeZone, company } = this.props;
        const url = `${apiUrl}getCommandBattIdList?groupInternalId=${groupInternalId}&commandId=${commandId}`;
        return ajax(url, 'GET', token, curLanguage, timeZone, company)
        //電池組ID 下拉選單(GET):
    ...
    //參數設定    
    //參數設定 查詢內阻設定BB:
    ajaxGetIRSetting = ({ batteryGroupId }) => {
        const { token, curLanguage, timeZone, company } = this.props;
    ...
    //參數設定 查詢時間週期設定BA:
    ajaxGetPeriodSetting = ({ batteryGroupId }) => {
        const { token, curLanguage, timeZone, company } = this.props;
        const url = `${apiUrl}getPeriodSetting?batteryGroupId=${batteryGroupId}`;
        return ajax(url, 'GET', token, curLanguage, timeZone, company)
        //參數設定 查詢時間週期設定BA:
        //https://www.gtething.tw/battery/getPeriodSetting?batteryGroupId=ZZ20B00039_0
    }
    //參數設定 查詢校正電壓B5:
    ajaxGetCorrectionVol = ({ batteryGroupId }) => {
        const { token, curLanguage, timeZone, company } = this.props;
        const url = `${apiUrl}getCorrectionVol?batteryGroupId=${batteryGroupId}`;
        return ajax(url, 'GET', token, curLanguage, timeZone, company)
        //參數設定 查詢校正電壓B5:
        //https://www.gtething.tw/battery/getCorrectionVol?batteryGroupId=ZZ20B00040_0
    }
    //參數設定 查詢校正內阻B3:
    ajaxGetCorrectionIR = ({ batteryGroupId }) => {
        const { token, curLanguage, timeZone, company } = this.props;
    ...
    //新增內阻設定BB(POST):
    ajaxAddIRSettingBatch = (postData) => {
        const { token, curLanguage, timeZone, company } = this.props;
    ...
    //新增時間週期設定BA(POST):
    ajaxAddPeriodSettingBatch = (postData) => {
        const { token, curLanguage, timeZone, company } = this.props;
    ...
    //新增校正電壓B5(POST):
    ajaxAddCorrectionVolTask = (postData) => {
        const { token, curLanguage, timeZone, company } = this.props;
    ...
    //新增校正內阻B3(POST):
    ajaxAddCorrectionIRTask = (postData) => {
        const { token, curLanguage, timeZone, company } = this.props;
