...
class Page1701 extends Component {
    ...
    HandleAddSumbit = () => {
        ...
        return this.ajaxAddUser(postData).then((response) => {
    ...
    HandleEditSumbit = () => {
        ...
        return this.ajaxUpdUser(postData).then((response) => {
            if (this.ajaxCancel) { return new Promise((resolve, reject) => (reject(''))); }//強制結束頁面
            if (Object.keys(response).length === 0) { return new Promise((resolve, reject) => (reject(''))); }
            const { code, msg } = response;
            if (code === '00' && msg) {
    ...
    HandleDelSumbit = () => {
        ...
        return this.ajaxDelUser(postData).then((response) => {
            if (this.ajaxCancel) { return new Promise((resolve, reject) => (reject(''))); }//強制結束頁面
            if (Object.keys(response).length === 0) { return new Promise((resolve, reject) => (reject(''))); }
            const { code, msg } = response;
            if (code === '00' && msg) {
    ...
    HandleRestPwdSumbit = () => {
        ...
        return this.ajaxResetPassword(postData).then((response) => {
            if (this.ajaxCancel) { return new Promise((resolve, reject) => (reject(''))); }//強制結束頁面
            if (Object.keys(response).length === 0) { return new Promise((resolve, reject) => (reject(''))); }
            const { msg } = response;
            this.setState({
            ...
            //     });
            // } else {
            //     console.error('使用者 ajaxResetPassword', response)
            //     this.setState({
            //         openDialog: false,
            //         openData: { ...initOpenData },
            //         openAlertMsg: true,//開啟訊息視窗
    ...
    onExportExcel = () => {
        ...
        this.ajaxGetUserInfoCheckExcel().then((response) => {
            ...
            if (code === '00') {
                ...
                const excelName = msg;
                const url = `${apiUrl}getUserInfo?account=${account}&type=${"csv"}`;
                const data = '';
    ...
    submitFilterData = (props) => {
        ...
        // 使用者資訊API(GET):
        return this.ajaxGetUserInfo().then((response) => {
            if (this.ajaxCancel) { return new Promise((resolve, reject) => (reject(''))); }//強制結束頁面
            if (Object.keys(response).length === 0) { return new Promise((resolve, reject) => (reject(''))); }
            const { code, msg } = response;
            if (code === '00' && msg) {
    ...
    ajaxGetUserInfo = () => {
        const { token, curLanguage, timeZone, company, account } = this.props;
        const url = `${apiUrl}getUserInfo?account=${account}&type=${''}`;
    ...
    ajaxGetUserInfoCheckExcel = () => {
        const { token, curLanguage, timeZone, company, account } = this.props;
        const url = `${apiUrl}getUserInfo?account=${account}&type=${"check"}`;
        return ajax(url, 'GET', token, curLanguage, timeZone, company)
        //使用者資訊:取EXCEL API(Get):
        //https://www.gtething.tw/battery/getUserInfo?account=&type=
    }
    //新增使用者API(POST):
    ajaxAddUser = (postData) => {
    ...
    //編輯使用者API(POST):
    ajaxUpdUser = (postData) => {
        const { token, curLanguage, timeZone, company } = this.props;
        const url = `${apiUrl}updUser`;
        return ajax(url, 'POST', token, curLanguage, timeZone, company, postData)
        //編輯使用者API(POST):
        //https://www.gtething.tw/battery/updUser
    }
    //刪除使用者API(POST):
    ajaxDelUser = (postData) => {
        const { token, curLanguage, timeZone, company } = this.props;
        const url = `${apiUrl}delUser`;
        return ajax(url, 'POST', token, curLanguage, timeZone, company, postData)
        //刪除使用者API(POST):
        //https://www.gtething.tw/battery/delUser
    }
    //重置密碼API(POST):
    ajaxResetPassword = (postData) => {
        const { token, curLanguage, timeZone, company } = this.props;
