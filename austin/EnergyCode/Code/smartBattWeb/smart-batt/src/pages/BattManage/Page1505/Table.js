...
class Table extends Component {
    ...
    render() {
        ...
        const { active, addEvent, deleteEvent, exportExcel, tableErrMsg, tableHeader, limits } = this.props;
        return (
            <Fragment>
                        ...
                        <tbody className='tbody'>
                            {tbody?.length > 0 ?
                                    ...
                                    .map((item, idx) => {
                                        return (
                                            <tr key={item.Seq}>
                                                ...
                                                { tableHeader[1].active && <td>{item.BatteryTypeName}</td>}
                                                {limits.Edit === 1 && Array.isArray(tableHeader) && tableHeader.length > 0 &&
                                                    <td className="text-center">
                                                        <FadeMenu
                                                            data={item}
                                ...
                                : <tr className="text-center td_btn-config">
                                    <td colSpan={limits.Edit === 1 ? "6" : "4"} >{tableErrMsg}</td>
                                </tr>}
                        </tbody>
                    </table>
                </div>
