...
class Page1502 extends Component {
    ...
    onExportExcel = () => {
        ...
        this.ajaxGetGroupManage(postData).then((response) => {
            if (this.ajaxCancel || this.importExcelCancel) { return; }//強制結束頁面
            if (Object.keys(response).length === 0) { return; }
            const { code, msg } = response;
            if (code === '00') {
                const excelName = msg;
                const url = `${apiUrl}getGroupManage`;
    ...
    handleAddSubmit = (data) => {
        ...
        this.ajaxAddBatteryGroup(postData).then((response) => {
    ...
    handleEditSubmit = (data) => {
        ...
        this.ajaxUpdBatteryGroup(postData).then((response) => {
            if (this.ajaxCancel) { return; }//強制結束頁面
            if (Object.keys(response).length === 0) { return; }
            const { code, msg } = response;
            if (code !== '00') {
    ...
    handleImportSubmit = (data) => {
        ...
        xhr.open('POST', `${apiUrl}importBatteryGroup`);   // 設置xhr得請求方式和url。
    ...
    handleDelSubmit = () => {
        ...
        this.ajaxDelBatteryGroup(postData).then((response) => {
            if (this.ajaxCancel) { return; }//強制結束頁面
            if (Object.keys(response).length === 0) { return; }
            const { code, msg } = response;
            if (code === '00' && msg) {
    ...
    //新增站台(POST):
    ajaxAddBatteryGroup = (postData) => {
        const { token, curLanguage, timeZone, company } = this.props;
        const url = `${apiUrl}addBatteryGroup`;
        return ajax(url, 'POST', token, curLanguage, timeZone, company, postData)
        //新增站台(POST):
    ...
    //修改站台(POST):
    ajaxUpdBatteryGroup = (postData) => {
        const { token, curLanguage, timeZone, company } = this.props;
        const url = `${apiUrl}updBatteryGroup`;
        return ajax(url, 'POST', token, curLanguage, timeZone, company, postData)
        //修改站台(POST):
    ...
    //刪除站台(POST):
    ajaxDelBatteryGroup = (postData) => {
        const { token, curLanguage, timeZone, company } = this.props;
        const url = `${apiUrl}delBatteryGroup`;
        return ajax(url, 'POST', token, curLanguage, timeZone, company, postData)
        //刪除站台(POST):
