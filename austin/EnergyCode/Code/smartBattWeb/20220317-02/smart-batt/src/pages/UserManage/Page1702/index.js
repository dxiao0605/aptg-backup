...
class Page1702 extends Component {
    ...
    HandleAddSumbit = () => {
        ...
        return this.ajaxAddRole(postData).then((response) => {
    ...
    HandleAuthoritySumbit = () => {
        ...
        return this.ajaxUpdRoleAuthority(postData).then((response) => {
            if (this.ajaxCancel) { return new Promise((resolve, reject) => (reject(''))); }//強制結束頁面
            if (Object.keys(response).length === 0) { return new Promise((resolve, reject) => (reject(''))); }
            const { code, msg } = response;
            if (code === '00' && msg) {
    ...
    HandleDelSumbit = () => {
        ...
        return this.ajaxDelRole(postData).then((response) => {
            if (this.ajaxCancel) { return new Promise((resolve, reject) => (reject(''))); }//強制結束頁面
            if (Object.keys(response).length === 0) { return new Promise((resolve, reject) => (reject(''))); }
            const { code, msg } = response;
            if (code === '00' && msg) {
    ...
    HandleAuthorityClick = ({ event, data, item: model }) => {
        ...
        this.ajaxGetRoleAuthority(data.RoleId).then((response) => {
    ...
    onExportExcel = () => {
        ...
        this.ajaxGetRoleInfoCheckExcel().then((response) => {
            ...
            if (code === '00') {
                ...
                const excelName = msg;
                const url = `${apiUrl}getRoleInfo?account=${account}&type=${"csv"}`;
                const data = '';
    ...
    submitFilterData = (props) => {
        ...
        // 角色資訊API(GET):
        return this.ajaxGetRoleInfo().then((response) => {
            if (this.ajaxCancel) { return new Promise((resolve, reject) => (reject(''))); }//強制結束頁面
            if (Object.keys(response).length === 0) { return new Promise((resolve, reject) => (reject(''))); }
            const { code, msg } = response;
            if (code === '00' && msg) {
    ...
    ajaxGetRoleInfo = () => {
        const { token, curLanguage, timeZone, company, account } = this.props;
        const url = `${apiUrl}getRoleInfo?account=${account}&type=${''}`;
        return ajax(url, 'GET', token, curLanguage, timeZone, company)
        //角色資訊API(GET):
        //https://www.gtething.tw/battery/getRoleInfo?account=&type=
    }
    //角色資訊:取EXCEL API(Get):
    ajaxGetRoleInfoCheckExcel = () => {
        const { token, curLanguage, timeZone, company, account } = this.props;
        const url = `${apiUrl}getRoleInfo?account=${account}&type=${"check"}`;
        return ajax(url, 'GET', token, curLanguage, timeZone, company)
        //角色資訊:取EXCEL API(Get):
        //https://www.gtething.tw/battery/getRoleInfo?account=&type=
    }
    //新增角色API(POST):
    ajaxAddRole = (postData) => {
        const { token, curLanguage, timeZone, company } = this.props;
        const url = `${apiUrl}addRole`;
        return ajax(url, 'POST', token, curLanguage, timeZone, company, postData)
        //新增角色API(POST):
        //https://www.gtething.tw/battery/addRole
    }
    //刪除角色API(POST):
    ajaxDelRole = (postData) => {
        const { token, curLanguage, timeZone, company } = this.props;
        const url = `${apiUrl}delRole`;
        return ajax(url, 'POST', token, curLanguage, timeZone, company, postData)
        //刪除角色API(POST):
        //https://www.gtething.tw/battery/delRole
    }
    //角色權限API(GET):
    ajaxGetRoleAuthority = (roleId) => {
        const { token, curLanguage, timeZone, company } = this.props;
        const url = `${apiUrl}getRoleAuthority?roleId=${roleId}`;
        return ajax(url, 'GET', token, curLanguage, timeZone, company)
        //角色權限API(GET):
        //https://www.gtething.tw/battery/getRoleAuthority?roleId=
    }
    //修改角色權限設定API(POST):
    ajaxUpdRoleAuthority = (postData) => {
        const { token, curLanguage, timeZone, company } = this.props;
        const url = `${apiUrl}updRoleAuthority`;
        return ajax(url, 'POST', token, curLanguage, timeZone, company, postData)
        //修改角色權限設定API(POST):
