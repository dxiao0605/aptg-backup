...
class AlertSolved extends Component {
    ...
    submitFilterData = (props) => {
        ...
        // get api 取得已解決告警
        this.fetchSolvedData(postData).then((response) => {
            if(response.code === '00' && response.msg) {
                ...
                this.setState({loading:false,data:[],IMPType:20,resultData:[],solvedData:[],solvedDataErrorMsg: response.msg})
            }
        })
    }
	...
	fetchSolvedData = (postData) => {
        const {token,company,curLanguage,timeZone} = this.props;
        const url = `${apiUrl}getAlert?eventStatus=6`;   //已解決
    ...
    exportSolvedDataExcel = () => {
        ...
        // 判斷是否有excel可執行下載
        this.fetchCheckSolvedDataEXCEL().then(response => {
            if(response.code === '00') {
                // type=csv 輸出CSV檔
                const {token,curLanguage,timeZone,company} = this.props;
                const exportURL = `${apiUrl}getAlert?eventStatus=6`;
    ...
    fetchCheckSolvedDataEXCEL = () => {
        const { token,curLanguage,timeZone,company} = this.props;
        const url = `${apiUrl}getAlert?eventStatus=6`;
