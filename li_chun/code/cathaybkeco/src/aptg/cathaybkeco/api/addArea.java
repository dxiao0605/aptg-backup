...
/**
 * Servlet implementation class addArea 新增區域資料
 */
@WebServlet("/addArea")
public class addArea extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private static final Logger logger = LogManager.getFormatterLogger(addArea.class.getName());
			...
			if (StringUtils.isNotBlank(token) && StringUtils.isNotBlank(req)) {
				...
				if (!ToolUtil.checkToken(token)) {
					...
					if(areaVO.isError()) {
						...
						if(areaDAO.checkAreaCode(null, areaVO.getAreaCode())) {
							rspJson.put("code", "22");
							rspJson.put("msg", "區域編號已存在");
						}else if(areaDAO.checkAreaName(null, areaVO.getAreaName())) {
							rspJson.put("code", "22");
							rspJson.put("msg", "區域名稱已存在");						
						}else {
							areaDAO.addArea(areaVO);
							ToolUtil.addLogRecord(areaVO.getUserName(), "23", "新增"+areaVO.getAreaCode()+"-"+areaVO.getAreaName());
