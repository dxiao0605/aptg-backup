...
import { NBSelectNames, ModelTable } from './InitDataFormat';
    ...
    return (
        <Fragment>
            ...
            >
                ...
                {
                    NBSelectNames.CompanyCodeI18nKey === title && ModelTable?.i18n16[isModel] && (Array.isArray(defSelectList) && defSelectList.length > 0 ?
                        defSelectList.map((item) => {
                            return item?.Value !== undefined &&
                                <MenuItem key={item.Value.toString()} value={item.Value.toString()}>{openI18nKey ? t(item.Label) : item.Label}</MenuItem>;
                        }) :
