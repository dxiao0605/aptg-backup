...
/**
 * Servlet implementation class getPowerRecordMaxDemand 電表最大需量
 */
@WebServlet("/getPowerRecordMaxDemand")
public class getPowerRecordMaxDemand extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private static final Logger logger = LogManager.getFormatterLogger(getPowerRecordMaxDemand.class.getName());

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public getPowerRecordMaxDemand() {
		super();
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		logger.debug("getPowerRecordMaxDemand start");
		...
		try {
			...
			logger.debug("DeviceID:" + deviceId + ",date:" + start + " ~ " + end);
			...
			if (StringUtils.isNotBlank(token)) {
				...
				if (!ToolUtil.checkToken(token)) {
					...
					powerAccountMaxDemandVO.setDeviceId(deviceId);
					...
					List<DynaBean> list = powerAccountMaxDemandDAO.getPowerRecordDemand(powerAccountMaxDemandVO);					
		...
		logger.debug("getPowerRecordMaxDemand end");
