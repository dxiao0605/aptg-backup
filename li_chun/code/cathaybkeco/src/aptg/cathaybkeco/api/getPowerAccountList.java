...
/**
 * Servlet implementation class getPowerAccountList 電號下拉選單
 */
@WebServlet("/getPowerAccountList")
public class getPowerAccountList extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private static final Logger logger = LogManager.getFormatterLogger(getPowerAccountList.class.getName());

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public getPowerAccountList() {
		super();
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		logger.debug("getPowerAccountList start");
		...
		try {
			...
			if (StringUtils.isNotBlank(token)) {
				...
				if (!ToolUtil.checkToken(token)) {
					...
					List<DynaBean> list = powerAccountDAO.getPowerAccountList(powerAccountVO);
					rspJson.put("code", "00");
		...
		logger.debug("getPowerAccountList end");
	...
	private JSONObject convertToJson(List<DynaBean> rows) throws Exception {
		...
		try {
			...
			for (DynaBean bean : rows) {
				list.put(bean.get("poweraccount"));
			}
			data.put("List", list);
		} catch (Exception e) {
