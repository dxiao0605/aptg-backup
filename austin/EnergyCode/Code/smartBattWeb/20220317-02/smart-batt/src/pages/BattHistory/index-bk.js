...
import {
	...
	initFilterDate,
} from "../../components/BattFilter/InitDataFormat"; //初始化格式
...
class BattHistory extends Component {
					...
					<div className='container-fuild'>
						<div className='col-12 p-0 mt-4 print-hide'>
							...
							<div className='d-inline-block'>
								<button
									className='btn-sm-b1 btn-outline-primary mr-2'
									onClick={this.resetFilterTrash}>
						...
						<div className='card batteryHistory-wrap print-hide'>
							<div className='col-12 col-md-4 batteryHistory-card'>
								<div className='col-12 align-top mb-2'>
									{/* 地圖左側地圖資訊 */}
									{battHistory && (
										<BattGroupIDTable battHistory={battHistory} />
									)}
								</div>
								<div className='col-12 map'>
									{
										//判斷是否有地圖資訊
										mapInfo.length <= 0 ? (
											<div className='p-4'>{mapInfoError}</div>
										) : (
											<LocationMarkerMap
												data={mapInfo}
												ApiKey={ApiKey}
							...
							<div className='col-12 col-md-8 batteryHistory-card'>
								{/* 地圖右側資訊 */}
								{battHistoryRecord.length <= 0 ? (
									<div>{battHistoryErrorMsg}</div>
								) : (
									<DataTable
	...
	submitFilterData = async (props) => {
			...
			if (response.code === "00" && response.msg) {
				this.setState({
					...
					tableHeader: [
						...
						{
							id: `${response.msg.Battery.IMPType}`,
							sortName: "",
							active: true,
						},
		...
		await this.fetchBattHistoryChart(postData).then((response) => {
			const { curLanguage, timeZone, timeZoneList } = this.props;
			const  tzLabel = timeZoneList.filter( item => parseInt(item.Value) === parseInt(timeZone))
			.map(elem => { 
				switch(elem.Label) {
					case 'Asia/Japan':
						return 'Asia/Tokyo'
					case 'Oceania/New Zealand':
			...
			if (response.code === "00") {
				const { IR, Vol, Temperature, RecTime } = response.msg;

				// 資料讀取成功並整理數值
				const data = getChartDatas(
					IR,
					Vol,
					Temperature,
					RecTime,
					this.props.isFilterRecordTimeData,
					curLanguage,
					tzLabel,
				);
				const minVol_decimals =
					response.msg.MinVol - Math.floor(response.msg.MinVol);
				const minVol =
					Math.floor(response.msg.MinVol) +
					Number((Math.floor(minVol_decimals * 10) / 10).toFixed(2));
				const maxVol = Number(
					(Math.ceil(response.msg.MaxVol * 20) / 20).toFixed(1)
				);
	...
	submitFilterItemTag = (name, item, idx) => {
		const { isFilterBatteryGroupIdData, isFilterRecordTimeData } = FilterNames;
		if (
			name === isFilterRecordTimeData &&
			isFilterRecordTimeData.Radio === "0"
		) {
			return;
		}
		let postData = {
	...
	fetchBattHistoryChart = () => {
		const { token, curLanguage, timeZone, company, battInternalId } =
			this.props;
...
const mapStateToProps = (state, ownProps) => {
	return {
		...
		// 篩選條件
		isFilterBatteryGroupIdData:
			state.BattFilterReducer.isFilterBatteryGroupIdData,
...
const mapDispatchToProps = (dispatch, ownProps) => {
	return {
		...
		// 篩選條件
		updateBatteryGroupIdBatt: (object) =>
			dispatch(updateBatteryGroupIdBatt(object)),
