...
import { setTreeFunctionId,resetTreeFunctionId } from '../../actions/MainNavAction';    //右側選單欄
import { setGroupID,setBattInternalIDAlert,updBattGroupTheader } from '../../actions/BattDataAction';   //表格表頭
import {
...
} from '../../actions/BattFilterAction';    //更新、清空篩選清單內容
...
import { fetchBattGroupData, fetchBASetting, fetchBBSetting, fetchCheckBattGroupDataEXCEL } from './getApi';
import { FilterNames, initFilterSelectData } from '../../components/BattFilter/InitDataFormat';//初始化格式
import { returnPostData, getExcelPostData } from './utils';
...
class BattGroup extends Component {
    ...
    submitFilterData = (props) => {
        ...
        // 取得電池數據資料(POST)
        fetchBattGroupData({query,postData}).then((response) => {// get api 多個電池群組
    ...
    handleSubmit = (code,list) => {
        ...
        if(code === '186'){
            ...
            fetchBASetting({query,postData:list}).then( (response) => {
            ...
            fetchBBSetting({query,postData:list}).then( (response) => {
				// 送出指令後,取得跳轉電池管理/電池參數設定值,回傳訊息
				this.afterSubmitResponse(response);
            })
        }
    ...
    exportBattGroupExcel = () => {
        ...
        const query = { token, curLanguage, timeZone, company };
        const postData = getExcelPostData({props:{
            isFilterCompanyData,isFilterCountryData,isFilterAreaData,isFilterGroupIdData,isFilterBatteryStatusData,
        },type:'check'});
        this.setState({
            openMsg:true,                      // 開啟訊息視窗
        ...
        // 判斷是否有excel可執行下載
        fetchCheckBattGroupDataEXCEL({query,postData}).then(response => {
            if (response.code === '00') {       // type=csv 輸出CSV檔
                // const {token, curLanguage, timeZone, company} = this.props;
                const {
                    isFilterCompanyData,isFilterCountryData,isFilterAreaData,isFilterGroupIdData,isFilterBatteryStatusData,
                } = this.props;
                const exportURL = `getBatteryGroup`;
