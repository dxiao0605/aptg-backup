...
import { ajaxGetIMPTypeCompany, ajaxGetCompanyLogo, ajaxGetLogoImages, ajaxDelLogo } from './getApi';
...
class Page1802 extends Component {
    ...
    handleRevert = () => {
        ...
        ajaxDelLogo({query,postData}).then((response) => {
    ...
    handleSubmit = () => {//確認送出        
        ...
        xhr.open('POST', `uploadLogo`);   // 設置xhr得請求方式和url。
    ...
    getIMPTypeCompany = () => {
        ...
        return ajaxGetIMPTypeCompany({query}).then((response) => {
            if (this.ajaxCancel) { return new Promise((resolve, reject) => (reject(''))); }//強制結束頁面
    ...
    getCompanyLogo = (companyCode) => {
        ...
        const { token, curLanguage, timeZone, company } = this.props;
        const query = { token, curLanguage, timeZone, company };
        ajaxGetCompanyLogo({query,companyCode}).then((response) => {
            if (this.ajaxCancel) { return new Promise((resolve, reject) => (reject(''))); }//強制結束頁面
            if (Object.keys(response).length === 0) { return new Promise((resolve, reject) => (reject(''))); }
            const { code, msg } = response;
            if (code === '00' && msg) {
    ...
    getLogoImages = (companyCode) => {
        ...
        const { token, curLanguage, timeZone, company } = this.props;
        const query = { token, curLanguage, timeZone, company };
        ajaxGetLogoImages({query,companyCode}).then(response => response.blob())
            .then(images => {
                this.defLogoBig.current.innerHTML = '';
                //圖片URL
