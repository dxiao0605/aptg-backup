 ...
 * 
 */
public class RatePlan4 extends RatePlanBase {
	...
	public RatePlan4(FcstChargeModel model) {
		// 是否夏月
	...
	public FcstChargeModel tpmCalculate(FcstChargeModel model) {
		...
		if (isSummer) {
			...
			this.tpmDemandPK = BigDecimal.ZERO;				// 台電計法本月最大尖峰需量
			this.tpmDemandSP = getMaxDemand(spList);		// 台電計法本月最大半尖峰需量 = 尖峰or半尖峰
			this.tpmDemandSatSP = BigDecimal.ZERO;			// 台電計法本月最大週六半需量
			this.tpmDemandOP = getMaxDemand(opList);		// 台電計法本月最大離峰需量 = 周六半or離峰
...
//		or
		 ...
		 * 	預測電費合計 #
		 */
		BigDecimal fcstTotalCharge = fcstTotalCharge(model.getBaseCharge(), model.getFcstUsageCharge(), BigDecimal.ZERO);
	...
	private void setFcstTPMMCEC() {
		if (isSummer) {
			this.fcstMCECPK = model.getFcstECO5MCECPK();									// 台電計法累計至今當月尖峰累積用電量 = 尖峰
			this.fcstMCECSP = model.getFcstECO5MCECSP();									// 台電計法累計至今當月半尖峰累積用電量 = 半尖峰
			...
			this.fcstMCECPK = BigDecimal.ZERO;												// 台電計法累計至今當月尖峰累積用電量 = 尖峰
			this.fcstMCECSP = model.getFcstECO5MCECPK().add(model.getFcstECO5MCECSP());		// 台電計法累計至今當月半尖峰累積用電量 = 半尖峰
			this.fcstMCECSatSP = BigDecimal.ZERO;											// 台電計法累計至今當月週六半尖峰累積用電量
			this.fcstMCECOP = model.getFcstECO5MCECSatSP().add(model.getFcstECO5MCECOP());	// 台電計法累計至今當月離峰累積用電量 = 周六半+離峰
			this.fcstMCEC = fcstMCECPK.add(fcstMCECSP).add(fcstMCECSatSP).add(fcstMCECOP);	// 台電計法累計至今當月總累積用電量
		}
	...
	public PowerRecordCollectionModel tpCollectionValue(PowerRecordCollectionModel record) {
		...
		if (isSummer) {
			...
			tpDemandSatSP = BigDecimal.ZERO;
			tpDemandOP = getMaxDemand(demandOPList);
			
			// 台電每日累積用電量
			tpdcecPK = record.getDCECPK();
			tpdcecSP = record.getDCECSP();
			tpdcecSatSP = BigDecimal.ZERO;
			tpdcecOP = record.getDCECSatSP().add(record.getDCECOP());
		...
		if (isSummer) {
			...
			tpmDemandSatSP = BigDecimal.ZERO;
			tpmDemandOP = getMaxDemand(demandOPList);

			// 台電每月累積用電量
			tpmcecPK = record.getMCECPK();
			tpmcecSP = record.getMCECSP();
			tpmcecSatSP = BigDecimal.ZERO;
			tpmcecOP = record.getMCECSatSP().add(record.getMCECOP());
			
		} else {
			// 台電每月最大需量
			tpmDemandPK = BigDecimal.ZERO;
