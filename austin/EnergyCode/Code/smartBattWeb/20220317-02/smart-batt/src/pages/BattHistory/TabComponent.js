        ...
        <CusTabPanel value={value} index={0} dir={theme.direction}>
          <div className="col-12 p-0">{/* 內阻曲線 */}
            <div className="card border-0 pl-2 pr-2">
                <h4 className="text-center w-100 print-only">{t('1601')} </h4>
                ...
                <div className="chart_content historyChart">
                    {   //判斷是否有電池狀態
                        (
                            ()=>{
                                if(chart === true && IRChart.length > 0) {
                                    return (
                                      <div>
                                        <div className='unit'>
                                          {
          ...
          <div className="card border-0 pl-2 pr-2">
              ...
              <div className="chart_content historyChart">
                  {   //判斷是否有電池狀態
                      (
                          ()=>{
                              if(chart === true && VolChart.length > 0) {
                                  return (
                                    <div>
                                      <div className='unit'>[V]</div>
                                      <CusChart data={VolChart} labels={RecTime} maxValue={MaxVol} minValue={MinVol} stepSize={stepSizeVol} showFloatValue={true}  unitStepSize={unitStepSize} windowSize={width} />
                                    </div>
                                  )
                              }else {
                                  return <div className="text-center p-4">{chartErrorMessage}</div>
            ...
            <div className="card border-0 pl-2 pr-2">
                ...
                <div className="chart_content historyChart">
                  {   //判斷是否有電池狀態
                      (
                          ()=>{
                              if(chart === true && TemperatureChart.length > 0) {
                                  return (
                                    <div>
