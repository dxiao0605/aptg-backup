...
class Table extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            tbody: [],
            row: 10,//目前每頁顯示筆數
            menuList: [CompareWindowns.Edit.model, CompareWindowns.Del.model, CompareWindowns.RestPwd.model],
        };
    }
    componentDidMount() {
        ...
        const newMenuList = this.state.menuList.map(item => {
            switch (item) {
                case CompareWindowns.RestPwd.model:
                    return (limits?.Button?.UserResetpw === 1) ? CompareWindowns.RestPwd.model : '';

                case CompareWindowns.Del.model:
                    return (limits?.Button?.UserDel === 1) ? CompareWindowns.Del.model : '';
            ...
            <Fragment>
                <div className="col-12 p-0 mb-1">
                    ...
                    <div className="col-12 col-sm-8 d-inline-block text-right p-0" style={{verticalAlign: 'bottom'}}>
                        <Toolbar isAddItem={limits?.Edit === 1 && limits?.Button?.UserCreate === 1 ? true : false} isExportExcel={true} addEvent={addEvent} exportExcel={exportExcel} />
                    </div>
                </div>
                            ...
                            {tbody?.length > 0 ?
                                tbody.slice(row * (active - 1), row * active)
                                    .map((item, idx) => {
                                        return (
                                            <tr key={item.Seq}>
                                                { tableHeader[0].active && <td>{item.UserName}</td>}
                                                { tableHeader[1].active && <td>{item.Account}</td>}
                                                { tableHeader[2].active && <td>{item.CompanyName}</td>}
                                                { tableHeader[3].active && <td>{item.RoleName}</td>}
                                                { tableHeader[4].active && <td>{item.LastLogin}</td>}
