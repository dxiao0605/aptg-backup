...
/**
 * Servlet implementation class updBatteryType 修改電池型號
 */
@WebServlet("/updBatteryType")
public class updBatteryType extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private static final Logger logger = LogManager.getFormatterLogger(updBatteryType.class.getName()); 

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public updBatteryType() {
		super();
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		logger.debug("updBatteryType start");
			...
			if (StringUtils.isNotBlank(token) && StringUtils.isNotBlank(req)) {
				...
				if (!ToolUtil.checkToken(token)) {
					...
					if(batteryTypeListVO.isError()) {
						...
						batteryTypeListDAO.updBatteryType(batteryTypeListVO);
		...
		logger.debug("updBatteryType end");
	...
	private BatteryTypeListVO parseJson(String json, ResourceBundle resource) throws Exception {
		...
		try {
			...
			if(ToolUtil.isNull(msg, "BatteryTypeCode")) {
				batteryTypeListVO.setError(true);
				batteryTypeListVO.setCode("24");
				batteryTypeListVO.setDescription(resource.getString("1030")+resource.getString("5008"));//必填欄位不能為空			
			}else {
				batteryTypeListVO.setBatteryTypeCode(msg.optString("BatteryTypeCode"));	
			}
			...
			if (checkBattTypeName(batteryTypeListVO)) {
				batteryTypeListVO.setError(true);
	...
	private boolean checkBattTypeName(BatteryTypeListVO batteryTypeListVO) throws Exception {
		...
		if (checkType != null && !checkType.isEmpty()) {
			DynaBean bean = checkType.get(0);
			if(batteryTypeListVO.getBatteryTypeName().equals(ObjectUtils.toString(bean.get("batterytypename")))) {
				return false;	
			}else {
				return true;	
			}
