...
/**
 * Servlet implementation class updControllerSetup 修改ECO5資料
 */
@WebServlet("/updControllerSetup")
public class updControllerSetup extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private static final Logger logger = LogManager.getFormatterLogger(updControllerSetup.class.getName());

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public updControllerSetup() {
		super();
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		logger.debug("updControllerSetup start");
			...
			if (StringUtils.isNotBlank(token) && StringUtils.isNotBlank(req)) {
				...
				if (!ToolUtil.checkToken(token)) {
					...
					if (controllerSetupVO.isError()) {
						...
						controllerSetupDAO.updControllerSetup(controllerSetupVO);
						ToolUtil.addLogRecord(controllerSetupVO.getUserName(), "8",
								"修改ECO-5帳號:" + controllerSetupVO.getEco5Account());
						rspJson.put("code", "00");
		...
		logger.debug("updControllerSetup end");
