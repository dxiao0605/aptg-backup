...
import { fetchAlertSetup, fetchUpdAlertSetup } from './getApi';
...
class AlertCondition extends Component {
    ...
    getAlertSetup = (value) => {
        ...
        const query = {token,language:curLanguage,timeZone,company:value}
        this.setState({loading: true})
        // 查看所選公司目前告警條件
        fetchAlertSetup({query}).then((response) => {
    ...
    handleSubmit = (e) => {
        ...
        const query = {token,language:curLanguage,timeZone,company:selectedCompany} ;
        ...
        // 清空訊息
        this.setState({successMsg: '', errorMsg: '',loading: true})
        fetchUpdAlertSetup({query,postData}).then( (response) => {
            if(response.code === '00'){
