...
import { fetchBattHistory, fetchBattHistoryChart, fetchCheckBattHistoryEXCEL } from './getApi';
import { getChartDatas, returnPostData } from './utils';
...
class BattHistory extends Component {
    ...
    submitFilterData = async(props) => {
        ...
        let postData;
        if (props === undefined) {
            postData = returnPostData({state:this.props,battId:battInternalId ,type:""});
        } else {
            postData = returnPostData({state:props,battId:battInternalId,type:""});
        }
        // get api 電池歷史
        await fetchBattHistory({query,postData}).then((response) => {
        ...
        await fetchBattHistoryChart({query,postData}).then( (response) => {
    ...
    exportBattHistoryExcel = () => {
        ...
        fetchCheckBattHistoryEXCEL({query,postData}).then(response => {
            if(response.code === '00') {
                const exportURL = `getBatteryHistory`;
