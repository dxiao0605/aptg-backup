...
class Page1505 extends Component {
    ...
    onExportExcel = () => {
        ...
        this.ajaxGetBatteryTypeList(postData).then((response) => {
            if (this.ajaxCancel || this.importExcelCancel) { return; }//強制結束頁面
            if (Object.keys(response).length === 0) { return; }
            const { code, msg } = response;
            if (code === '00') {
                const excelName = msg;
                const url = `${apiUrl}getBatteryTypeList`;
    ...
    handleAddSubmit = (data) => {
        ...
        this.ajaxAddBatteryType(postData).then((response) => {
    ...
    handleEditSubmit = (data) => {
        ...
        this.ajaxUpdBatteryType(postData).then((response) => {
            if (this.ajaxCancel) { return; }//強制結束頁面
            if (Object.keys(response).length === 0) { return; }
            const { code, msg } = response;
            if (code === '00' && msg) {
    ...
    handleDelSubmit = () => {
        ...
        this.ajaxDelBatteryType(postData).then((response) => {
            if (this.ajaxCancel) { return; }//強制結束頁面
            if (Object.keys(response).length === 0) { return; }
            const { code, msg } = response;
            if (code === '00' && msg) {
    ...
    //電池類型API(POST):(含EXCEL)
    ajaxGetBatteryTypeList = (postData) => {
        const { token, curLanguage, timeZone, company } = this.props;
        const url = `${apiUrl}getBatteryTypeList`;
    ...
    //新增電池型號API:
    ajaxAddBatteryType = (postData) => {
        const { token, curLanguage, timeZone, company } = this.props;
        const url = `${apiUrl}addBatteryType`;
        return ajax(url, 'POST', token, curLanguage, timeZone, company, postData)
        //新增電池型號API:
    ...
    //修改電池型號API:
    ajaxUpdBatteryType = (postData) => {
        const { token, curLanguage, timeZone, company } = this.props;
        const url = `${apiUrl}updBatteryType`;
        return ajax(url, 'POST', token, curLanguage, timeZone, company, postData)
        //修改電池型號API:
    ...
    //刪除電池型號API:
    ajaxDelBatteryType = (postData) => {
        const { token, curLanguage, timeZone, company } = this.props;
        const url = `${apiUrl}delBatteryType`;
        return ajax(url, 'POST', token, curLanguage, timeZone, company, postData)
        //刪除電池型號API:
