...
/**
 * Servlet implementation class getCorrectionIR 校正內阻(B3)
 */
@WebServlet("/getCorrectionIR")
public class getCorrectionIR extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private static final Logger logger = LogManager.getFormatterLogger(getCorrectionIR.class.getName());

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public getCorrectionIR() {
		super();
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		logger.debug("getCorrectionIR start");
		...
		try {
			...
			if (StringUtils.isNotBlank(token) && StringUtils.isNotBlank(batteryGroupId)) {
				...
				if (!ToolUtil.checkToken(token)) {
					...
					batteryVO.setCommandID("B3");
					batteryVO.setCategory("1");
		...
		logger.debug("getCorrectionIR end");
	}
	...
	private JSONObject convertToJson(List<DynaBean> rows,String timezone) throws Exception {
		...
		try {			
			...
			int setCo = rows.size();			
			int defaultIR = ToolUtil.parseInt(SysConfig.getInstance().getIR());
			
			JSONArray arr = new JSONArray();
			if(count!=setCo) {
				for(int i=0; i<count; i++) {
					arr.put(defaultIR);//預設
				}
