...
import { ajaxDelFilter , ajaxGetFilter, ajaxGetNBHistoryFilter } from '../../../components/FilterDrawer/utils';
...
class Filter extends Component {
                ...
                <ul className={`filter overflowY mb-0 ${navigator.userAgent.indexOf('Windows') > 0 ? 'Windows' : 'Mac'}`}>
                    {
                        ...
                        <li>{/* 異動者公司 */}
                            <FilterItemHeader
                                ...
                                onUpdateChainSelectList={() => this.onUpdateChainSelectList('CompanyValue', getSelectObj(isFilterCompanyList))}
                    ...
                    <li>{/* 通訊序號 */}
                        <FilterItemHeader
                            ...
                            onUpdateChainSelectList={() => this.onUpdateChainSelectList('NBIDValue', getSelectObj(isFilterNBIDList))}
    ...
    //異動記錄篩選清單API:
    getNBHistoryFilter = (defaultCompany, defaultNBID, defaultModifyItem,) => {
        const { token, curLanguage, timeZone, company } = this.props;
        const query = { token, curLanguage, timeZone, company };
        ajaxGetNBHistoryFilter({query}).then((response) => {
