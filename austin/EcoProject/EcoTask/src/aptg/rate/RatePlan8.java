 ...
 * 
 */
public class RatePlan8 extends RatePlanBase {
	...
	private static final int RatePlan = 8;
	
	public RatePlan8() {}
	
	public RatePlan8(FcstChargeModel model) {
		// 是否夏月
	...
	public FcstChargeModel tpmCalculate(FcstChargeModel model) {
		 ...
		 */
//		if (isSummer) {
			this.tpmDemandPK = getMaxDemand(pkList);		// 台電計法本月最大尖峰需量 = 尖峰+半尖峰
...
//		} else {
//			this.tpmDemandPK = model.getMDemandPK();		// 台電計法本月最大尖峰需量 = 尖峰+半尖峰
//			this.tpmDemandSP = model.getMDemandSP();				// 台電計法本月最大半尖峰需量
//			this.tpmDemandSatSP = model.getMDemandSatSP();	// 台電計法本月最大週六半需量 = 周六半
//			this.tpmDemandOP = model.getMDemandOP();		// 台電計法本月最大離峰需量 = 離峰
//			
//			this.tpmcecPK = model.getMCECPK();				// 台電計法累計至今當月尖峰累積用電量 = 尖峰+半尖峰
//			this.tpmcecSP = model.getMCECSP();										// 台電計法累計至今當月半尖峰累積用電量
//			this.tpmcecSatSP = model.getMCECSatSP();								// 台電計法累計至今當月週六半尖峰累積用電量 = 周六半
//			this.tpmcecOP = model.getMCECOP();										// 台電計法累計至今當月離峰累積用電量 = 離峰
//			this.tpmCEC = tpmcecPK.add(tpmcecSP).add(tpmcecSatSP).add(tpmcecOP);	// 台電計法累計至今當月總累積用電量
//		}
...
//		if (isSummer) {
			// 台電每日最大需量
			tpDemandPK = getMaxDemand(demandPKList);
			tpDemandSP = BigDecimal.ZERO;
			tpDemandSatSP = record.getDemandSatSP();
			tpDemandOP = record.getDemandOP();
			
			// 台電每日累積用電量
			tpdcecPK = record.getDCECPK().add(record.getDCECSP());
			tpdcecSP = BigDecimal.ZERO;
...
//		if (isSummer) {
			// 台電每月最大需量
			tpmDemandPK = getMaxDemand(demandPKList);
			tpmDemandSP = BigDecimal.ZERO;
			tpmDemandSatSP = record.getMDemandSatSP();
			tpmDemandOP = record.getMDemandOP();

			// 台電每月累積用電量
			tpmcecPK = record.getMCECPK().add(record.getMCECSP());
			tpmcecSP = BigDecimal.ZERO;
