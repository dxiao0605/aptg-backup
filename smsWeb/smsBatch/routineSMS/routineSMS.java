...
public class routineSMS {
	...
	private static void loop() {
		...
		try {
			...
			sql1 = "select trim(concat(replace(SUBSTRING_INDEX(SUBSTRING_INDEX(time_spec, 'time_spec-', -1),';',1),'.',':'),':00')) as t_time,s_id,sender,msisdn_lst,trim(msg_content) as msg_content";
			...
			sql1 += " and DATE_FORMAT(now(),'%Y/%m/%d') <= SUBSTRING_INDEX(SUBSTRING_INDEX(time_spec, 'period_spec-', -1),'.',-1) ";
			sql1 +=" and concat(s_id,'#',sender,'#',DATE_FORMAT(now(),'%Y-%m-%d'),' ',trim(concat(replace(SUBSTRING_INDEX(SUBSTRING_INDEX(time_spec, 'time_spec-', -1),';',1),'.',':'),':00'))) not in (select concat(sms_box_seq,'#',a_num,'#',target_time) from msgweb.sms_send_msg)";
