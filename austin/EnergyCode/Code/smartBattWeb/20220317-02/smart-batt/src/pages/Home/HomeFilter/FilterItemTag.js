                ...
                <div className="d-inline-block">
                    {
                        ...
                        isFilterEventTypeCodeData.isButtonList.map((item, idx) => {
                            return <button key={item.Value} className="btn-sm-b1 btn-outline-primary mr-2">
                                <i className="fas fa-window-close mr-1" onClick={() => this.props.submitFilterItemTag(FilterNames.isFilterEventTypeCodeData, item, idx)} />{item.Label}
                            </button>
                        })
                    }
                    {
                    ...
                    {//不可刪除欄位
                        ...
                        isFilterRecordTimeData.isButtonList.map((item, idx) => {
                            return <button key={item.Value} className="btn-sm-b1 btn-outline-primary mr-2"><Trans i18nKey={item.Label} /></button>
                        })
                    }
                </div>
            </Fragment>
