...
import { checkObject } from "../FilterDrawer/utils"; //整理checkbox狀態
import FilterItemSelect from "../FilterDrawer/FilterItemSelect"; //每筆Item的Select
import {
	FilterNames,
	FilterNames_API,
	initFilterSelectData,
} from "./InitDataFormat"; //初始化格式

const apiUrl = apipath();
// console.log("Mac",navigator.userAgent.indexOf('Mac') > 0)
// console.log('Windows',navigator.userAgent.indexOf('Windows') > 0);
								...
								<FilterItemHeader
									...
									onUpdateChainSelectList={() =>
										this.onUpdateChainSelectList(
											...
											this.getSelectObj(isFilterCompanyList)
										)
									}
									onUpdateList={(object) =>
						...
						<FilterItemHeader
							...
							onUpdateChainSelectList={() =>
								this.onUpdateChainSelectList(
									...
									this.getSelectObj(isFilterCountryList)
								)
							}
							onUpdateList={(object) =>
						...
						<FilterItemHeader
							...
							onUpdateChainSelectList={() =>
								this.onUpdateChainSelectList(
									...
									this.getSelectObj(isFilterAreaList)
								)
							}
							onUpdateList={(object) =>
	...
	delFilter = (FilterID) => {
		this.ajaxDelFilter({ FilterID }).then((response) => {
			...
			if (code === "00" && msg) {
				...
				console.error("電池數據-篩選儲存清單", response);
				this.props.alertFilterMsgOpen(msg);
			}
		});
	};
