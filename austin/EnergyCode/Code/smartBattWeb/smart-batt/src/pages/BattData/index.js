...
import { locpath } from "../../utils/ajax";
...
import { returnPostData } from './filterUtils';
...
import { 
	getBattData, 			// get api 取得電池組ID(POST)
	fetchBASetting, 		// api 變更BA設定清單(POST)
	fetchBBSetting, 		// api 變更BB設定清單(POST)
	fetchB3Setting, 		// api 變更B3設定清單(POST)
	fetchB5Setting, 		// api 變更B5設定清單(POST)
	fetchGroupSetting,		// api 變更站台編輯(POST)
	fetchBatterySetting,	// api 變更電池組編輯(POST)
	fetchCheckBattDataEXCEL,// type=check檢查有EXCEL無資料
} from "./getApi";
...
class BattData extends Component {
    ...
    submitFilterData = (props) => {
        ...
        // 取得電池數據資料(POST)
		getBattData({query,postData}).then((response) => {
			// get api 單一電池組
	...
	handleSubmit = async(code,list) => {
		...
		if (code === "186") {// 發送批次BA指令並回傳BASetting訊息
			...
			await fetchBASetting({query,postData:list}).then((response) => {
				// 送出指令後,取得跳轉電池管理/電池參數設定值,回傳訊息
				this.afterSubmitResponse(response);
			})
		} else if (code === "187") {// 發送批次BB指令並回傳BBSetting訊息
			await this.setState({msgTitle: '1558'})// 訊息標題(指令結果)
			await fetchBBSetting({query,postData:list}).then((response) => {
				// 送出指令後,取得跳轉電池管理/電池參數設定值,回傳訊息
				this.afterSubmitResponse(response);
			})
		} else if (code === "179") {// 發送批次B3指令並回傳B3Setting訊息
			await this.setState({msgTitle: '1558'})// 訊息標題(指令結果)
			await fetchB3Setting({query,postData:list}).then((response) => {
				// 送出指令後,取得跳轉電池管理/電池參數設定值,回傳訊息
				this.afterSubmitResponse(response);
			})
		} else if (code === "181") {// 發送批次B5指令並回傳B5Setting訊息
			await this.setState({msgTitle: '1558'})// 訊息標題(指令結果)
			await fetchB5Setting({query,postData:list}).then((response) => {
				// 送出指令後,取得跳轉電池管理/電池參數設定值,回傳訊息
				this.afterSubmitResponse(response);
			})
			...
			await fetchGroupSetting({query,postData:list}).then(async(response) => {
				if (response.code === "00") {
			...
			await this.setState({msgTitle:"1089"})// 訊息標題(處理結果)
			await fetchBatterySetting(newList).then(async(response) => {
				if (response.code === "00") {
					await this.submitFilterData()
					await this.setState({
						TaskId: [],
	...
	exportBattDataExcel = () => {
		...
		const postData = getPostData({props:{
            isFilterCompanyData,isFilterCountryData,isFilterAreaData,
			isFilterGroupIdData,
			isFilterBatteryStatusData},type:'csv'})
		// 判斷是否有excel可執行下載
		fetchCheckBattDataEXCEL({query,postData}).then((response) => {
			if (response.code === "00") {// type=csv 輸出CSV檔
				const {token, curLanguage, timeZone, company} = this.props;
				const exportURL = `getBattery`;
