...
/**
 * Servlet implementation class addAdminSetup 新增帳號資料
 */
@WebServlet("/addAdminSetup")
public class addAdminSetup extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private static final Logger logger = LogManager.getFormatterLogger(addAdminSetup.class.getName());
	
	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public addAdminSetup() {
		super();
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response)
			...
			if (StringUtils.isNotBlank(token) && StringUtils.isNotBlank(req)) {
				...
				if (!ToolUtil.checkToken(token)) {
					...
					if(adminSetupVO.isError()) {
						...
						if(adminSetupDAO.checkAdminSetup(adminSetupVO.getAccount())) {
							rspJson.put("code", "22");
							rspJson.put("msg", "帳號已存在");
						}else {
							adminSetupDAO.addAdminSetup(adminSetupVO);
							ToolUtil.addLogRecord(adminSetupVO.getUserName(), "17", "新增"+adminSetupVO.getAccount()+"帳號資訊");
							...
							msg.put("Message", "Insert Success");
							rspJson.put("msg", msg);
							rspJson.put("code", "00");
						}
					}
		...
		logger.debug("addAdminSetup end");
	...
	private AdminSetupVO parseJson(String json) throws Exception {
		...
		try {
			...
			if(ToolUtil.isNull(request, "Account")) {
				...
				return adminSetupVO;
			}else if(!ToolUtil.lengthCheck(request.optString("Account"), 50)) {
				adminSetupVO.setError(true);
				adminSetupVO.setCode("12");
				adminSetupVO.setDescription("帳號超過長度限制");
				return adminSetupVO;
			}else {
			...
			adminSetupVO.setEnabled("0");//0:停用, 1:啟動
			adminSetupVO.setSuspend("0");//0:非停權, 1:停權
		} catch (Exception e) {
