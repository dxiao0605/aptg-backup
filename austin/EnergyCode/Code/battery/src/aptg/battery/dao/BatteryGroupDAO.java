...
//		sql.append(" order by GroupName  ");
	...
	public void delBatteryGroup(BatteryGroupVO batteryGroupVO) throws Exception {	
		...
		try {
			...
			String newStartTime = sdf.format(new Date());
			
			// insert NBID Default NBGroupHis
			StringBuffer sql4 = new StringBuffer();
			sql4.append(" insert into NBGroupHis ");
			sql4.append(" (NBID, GroupInternalID, StartTime, EndTime, CreateUserName, UpdateUserName) ");
			sql4.append(" select ");
			sql4.append(" NBID, ?, ?, '2038-01-01 00:00:00', ?, ? from NBGroupHis ");
			sql4.append(" where GroupInternalID=? and StartTime<=now() and EndTime>now() ");
			ps4 = connection.prepareStatement(sql4.toString());
			for(BatteryGroupVO dataVO : batteryGroupVO.getDataList()) {
				ps4.setString(1, dataVO.getDefaultInternalId());	// default GroupInternalID
				ps4.setString(2, newStartTime);
				ps4.setString(3, batteryGroupVO.getUserName());
				ps4.setString(4, batteryGroupVO.getUserName());
				ps4.setString(5, dataVO.getGroupInternalId());	// 刪除前的GroupInternalID
			...
			// update deleted NBID NBGroupHis EndTime
			StringBuffer sql5 = new StringBuffer();
			sql5.append(" update NBGroupHis set EndTime=? where GroupInternalID=? and StartTime<=now() and EndTime>now() ");
			ps5 = connection.prepareStatement(sql5.toString());
			for(BatteryGroupVO dataVO : batteryGroupVO.getDataList()) {
				ps5.setString(1, newStartTime);
				ps5.setString(2, dataVO.getGroupInternalId());
	...
	public List<DynaBean> getDefaultGroup(BatteryGroupVO batteryGroupVO) throws Exception {
		...
		sql.append(" b.CompanyCode, b.GroupID ");
		sql.append(" , n.NBID ");	// Austin
		sql.append(" from BatteryGroup b, BatteryGroup d ");
		sql.append(" , NBList n ");	// Austin
		sql.append(" where b.CompanyCode = d.CompanyCode ");		
		sql.append(" and d.DefaultGroup = 0 ");
		sql.append(" and b.SeqNo=n.GroupInternalID ");	// Austin
	 ...
	 * @throws Exception
	 */
	public List<DynaBean> getCommandList(BatteryGroupVO batteryGroupVO,String commandId) throws Exception {
		List<String> parameterList = new ArrayList<String>();
		StringBuffer sql = new StringBuffer();

		
		if(commandId.equals("B3") || commandId.equals("B5")) {
		...
		// David 20220307 for BA BB command
		else
		{
			sql.append(" select distinct c.CompanyCode, bg.Country, bg.Area, bg.GroupID, bg.GroupName, bg.SeqNo ");
			sql.append(" from Company c, BatteryGroup bg ");
