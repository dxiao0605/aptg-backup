...
public class getAlert extends HttpServlet {
	...
	private AlertDataBean processData(List<DynaBean> rows, int impType, String language, String timezone) throws Exception {
		 ...
		 */
		SimpleDateFormat solveSDF = ToolUtil.getDateFormat("yyyy/MM/dd HH:mm:ss Z", timezone);
		/*
		 * add by Austin (2021/12/02)
		 * =============== End ===============
		 */
		...
		try {
			...
			for(int i=0; i<rows.size(); i++) {							
				...
				if(alertMap.containsKey(eventSeq)) {
					...
					alertBean.setCloseTime(ToolUtil.dateFormat(bean.get("closetime"), solveSDF));//解決時間	// edit by Austin (2021/12/02)
					 ...
					 * add by Austin (2021/12/02)
					 * 警戒、需更換: 顯示判定值1、判定值2
					 * 離線、溫度: 顯示判定值1，不顯示判定值2的部分
					 */
					if (eventstatus.equals("5")) {	// 未解決
						if (eventtype.equals("4") || eventtype.equals("25")) {	// 告警case: 離線 or 溫度
							alertBean.setAlert1("");	// 判定值1 (網頁會抓離線判斷時間、溫度限制)
							alertBean.setAlert2("");	// 不顯示
						}
					}
					/*
					 * add by Austin (2021/12/02)
