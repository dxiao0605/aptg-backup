...
import { 
    getUnsolvedData,            // get api 取得未解決告警
    fetchCloseAlert,            // 關閉多個告警API(POST) 暫無
    fetchCheckunsolvedDataEXCEL,// 判斷是否有excel可執行下載
} from './getAPi';
...
import { FilterNames, initFilterDate, initFilterSelectData } from './AlertUnSolvedFilter/InitDataFormat';//初始化格式&returnPostData function
import { returnPostData } from './AlertUnSolvedFilter/utils';
...
class AlertUnsolved extends Component {
    ...
    submitFilterData = (props) => {
        ...
        let postData;
        if (props === undefined) {
            postData = returnPostData(this.props, "",this.props.battInternalId);
        } else {
            postData = returnPostData(props, "",this.props.battInternalId);
        }
        // get api 取得未解決告警
        getUnsolvedData({query,postData}).then((response) => {
    ...
    exportunsolvedDataExcel = () => {
        const { token,curLanguage,timeZone,company } = this.props;
        const query = { token,curLanguage,timeZone,company};
        const postData = returnPostData(this.props,'check',this.props.battInternalId);
        this.setState({
            openMsg: true,                      // 開啟訊息視窗
            message: "1091",                    // 填寫訊息
        ...
        // 判斷是否有excel可執行下載
        fetchCheckunsolvedDataEXCEL({query, postData}).then(response => {
            if(response.code === '00') {
                // type=csv 輸出CSV檔
                const {token,curLanguage,timeZone,company} = this.props;
                const exportURL = `getAlert?eventStatus=5`;
                const postData = returnPostData(this.props,'csv',this.props.battInternalId);
