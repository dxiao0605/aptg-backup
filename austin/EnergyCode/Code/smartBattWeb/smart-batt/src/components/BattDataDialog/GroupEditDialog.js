...
import { fetchGroupSetup, fetchNBList } from './getApi';
...
class GroupEditDialog extends Component {
    ...
    getGroupSetup = (GroupInternalID,NBID) => {
        const { token,timeZone,curLanguage } = this.props;
        const query = { token,company:this.props.data[0].CompanyCode,timeZone,language:curLanguage}
        fetchGroupSetup({query}).then( async(response)=>{
            if(response.code === '00'){
    ...
    getNBList = (GroupInternalId,nbid) => {
        ...
        const query = { token,company:userCompany ,timeZone,language:curLanguage};
        fetchNBList({query,GroupInternalId,nbid}).then( (response) => {
            if(response.code === '00') {
                this.setState({
...
GroupEditDialog.propTypes = {
    ...
    handleGroupListNBChange: PropTypes.func,
}
