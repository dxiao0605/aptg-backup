...
import { ajaxSaveFilter } from '../../components/FilterDrawer/utils';           // 儲存篩選API(POST)
import { FilterNames,initFilterDate } from '../../components/BattFilter/InitDataFormat';         // 初始化格式
import { fetchBattHistoryData } from './getApi';
...
class BattHistoryManage extends Component {
    ...
    submitFilterData = (props) => {
        ...
        // 取得電池歷史篩選(POST)
        fetchBattHistoryData({query,postData}).then((response) => {// get api 多個電池群組
            this.setState({
                isFilterOpen: false,
            })
    ...
    exportBattHistoryExcel = (list) => {
        ...
        // 執行下載excel
        const {token, curLanguage, timeZone, company} = this.props;
        const exportURL = `getBatteryHistoryExcel`;
        const postData = returnPostData(this.props, "excel");
