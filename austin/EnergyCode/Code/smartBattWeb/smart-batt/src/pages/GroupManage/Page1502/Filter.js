...
import { ajaxDelFilter, ajaxGetFilter, ajaxGetGroupManageFilter } from '../../../components/FilterDrawer/utils';
...
class Filter extends Component {
    ...
    componentDidMount() {
        ...
        this.getGroupManageFilter(isFilterCompanyData, isFilterCountryData, isFilterAreaData, isFilterGroupIdData);//取選單資料
    ...
    //電池組管理篩選清單API:
    getGroupManageFilter = (defaultCompany, defaultCountry, defaultArea, defaultGroupId,
        // defaultGroupName
    ) => {
        const { token, curLanguage, timeZone, company } = this.props;
        const query = { token, curLanguage, timeZone, company };
        ajaxGetGroupManageFilter({query}).then((response) => {
    ...
    delFilter = (FilterID) => {
        ...
        ajaxDelFilter({query,postData}).then((response) => {
            ...
            if (msg) {
                ...
                console.error('站台管理 ajaxDelFilter', response);
            }
        });
    }
}
