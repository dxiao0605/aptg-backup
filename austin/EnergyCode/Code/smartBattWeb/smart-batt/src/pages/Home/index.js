...
import { returnPostData } from './utils';
import { fetchMapInfo, fetchBattStateChartData, fetchBattStateLineChartData,fetchCheckStatusNowDataExcel, fetchCheckStatusDataEXCEL } from './getApi';
...
class Home extends Component {
    ...
    submitFilterData = (props) => {
        ...
        // 取得地圖群組資訊資料(POST)
        fetchMapInfo({query,postData}).then((response) => {
    ...
    getBattStateChartData = (props) => {
        ...
        // alert('refresh 電池狀態')
        fetchBattStateChartData({query,postData}).then((response) => {
            if (response.code === '00' && response.msg) {
    ...
    getBattStateLineChartData = (props) => {
        ...
        fetchBattStateLineChartData({query,postData}).then((response) => {
            if (response.code === '00' && response.msg) {
    ...
    exportStatusNowDataExcel = () => {
        ...
        fetchCheckStatusNowDataExcel({query,postData}).then(response => {
            if (response.code === '00') {
                // type=csv 輸出CSV檔
                // const { token, curLanguage, timeZone, company } = this.props;
                const exportURL = `getStatusNow`;
    ...
    exportStatusDataExcel = () => {
        ...
        const query = { token, curLanguage, timeZone, company };
        const postData = returnPostData(this.props, 'check');
        fetchCheckStatusDataEXCEL({query,postData}).then(response => {
            if (response.code === '00') {
                // type=csv 輸出CSV檔
                const { token, curLanguage, timeZone, company } = this.props;
                const exportURL = `getStatus?type=csv`;
                const excelName = response.msg;
                const postData = returnPostData(this.props, 'csv');
                exportExcel(token, curLanguage, timeZone, company, exportURL, excelName, postData, () => {
                    this.setState({
