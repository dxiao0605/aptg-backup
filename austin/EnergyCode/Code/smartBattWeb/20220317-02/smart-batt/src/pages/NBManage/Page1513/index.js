...
class Page1513 extends Component {
    ...
    handleNotSentAndSentSumit = (data) => {
        ...
        // 分配通訊序號(POST):
        return this.ajaxUpdNBListAllocate(postData).then((response) => {
    ...
    ajaxGetCompanyList = () => {
        ...
        const url = `${apiUrl}getCompanyList?admin=0`;
        return ajax(url, 'GET', token, curLanguage, timeZone, company)
        //取選擇公司清單(GET):
        //https://www.gtething.tw/battery/getCompanyList?admin=0
    }
    //通訊序號列表(GET):
    ajaxGetNBList = (data) => {
        const { token, curLanguage, timeZone, company } = this.props;
    ...
    //分配通訊序號(POST):
    ajaxUpdNBListAllocate = (postData) => {
        const { token, curLanguage, timeZone, company } = this.props;
        const url = `${apiUrl}updNBListAllocate`;
        return ajax(url, 'POST', token, curLanguage, timeZone, company, postData)
        //分配通訊序號(POST):
        //https://www.gtething.tw/battery/updNBListAllocate
    }
    //修改通訊序號狀態/刪除通訊序號(POST):
