...
/**
 * Servlet implementation class addElectricityPrice 修改歷年電價表
 */
@WebServlet("/updElectricityPrice")
public class updElectricityPrice extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private static final Logger logger = LogManager.getFormatterLogger(updElectricityPrice.class.getName());

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public updElectricityPrice() {
		super();
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		logger.debug("updElectricityPrice start");
			...
			if (StringUtils.isNotBlank(token) && StringUtils.isNotBlank(req)) {
				...
				if (!ToolUtil.checkToken(token)) {
					...
					if(electricityPriceVO.isError()) {
						...
						electricityPriceDAO.updElectricityPrice(electricityPriceVO);
						ToolUtil.addLogRecord(electricityPriceVO.getUserName(), "12", "修改電費單價設定，Date:"+electricityPriceVO.getYear()+electricityPriceVO.getMonth()+",RatePlanCode:"+electricityPriceVO.getRatePlanCode());
						rspJson.put("code", "00");
						...
						data.put("Message", "Update Success");
		...
		logger.debug("updElectricityPrice end");
		...
		try {
			...
			while(iterator.hasNext()){
				...
				if (ToolUtil.isNull(msg, key) && !"UserName".equals(key)) {
					...
					break;
				}else if (!"UserName".equals(key) && !ToolUtil.numberCheck(msg.optString(key))) {
					electricityPriceVO.setError(true);
					electricityPriceVO.setCode("13");
					electricityPriceVO.setDescription("數字格式錯誤");
					break;
				}else if (!"UserName".equals(key) && !ToolUtil.numberLengthCheck(msg.optString(key), 5, 3)) {
					electricityPriceVO.setError(true);
			...
			electricityPriceVO.setNewYear(String.valueOf(msg.optInt("NewYear")+1911));
			electricityPriceVO.setNewMonth(msg.optString("NewMonth"));
			...
			if("1".equals(ratePlanCode)) {
				//表燈營業用
				electricityPriceVO.setLampBPrice1(msg.optString("LampBPrice1"));
