...
import { ajaxGetBattManage, ajaxUpdBattery, ajaxGetBattManageExcel } from './getApi';
...
class Page1501 extends Component {
    ...
    handleEditSubmit = (data) => {
        ...
        ajaxUpdBattery({query,postData}).then((response) => {
    ...
    onExportExcel = () => {
        ...
        ajaxGetBattManageExcel({query,postData}).then((response) => {
            ...
            if (code === '00') {
                ...
                const data = returnPostData(this.props, "csv");
                const { token, curLanguage, timeZone, company } = this.props;
    ...
    submitFilterData = (props) => {
        ...
        // 電池組管理API
        ajaxGetBattManage({query,postData}).then((response) => {
            if (this.ajaxCancel) { return; }//強制結束頁面
            if (Object.keys(response).length === 0) { return; }
            const { code, msg } = response;
            if (code === '00' && msg) {
