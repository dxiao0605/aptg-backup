...
public class PowerRecordDao extends BaseDao2 {
	...
	public List<Integer> insertPowerRecord(PowerRecordModel model) throws SQLException {
	...
	public int insertPowerRecordWithPartition(PowerRecordModel model) throws SQLException {
		List<String> sqlList = new ArrayList<>();
		
		String createTime = ToolUtil.getInstance().convertDateToString(new Date(), "yyyy-MM-dd HH:mm:ss");
		...
		ps.setBigDecimal(37, model.getHighCECSP());
		ps.setBigDecimal(38, model.getHighCECOP());
		ps.setString(39, createTime);
		ps.addBatch();
		sqlList.add(ps.toString());
		psList.add(ps);
		
		String sql2 = "INSERT INTO PowerRecordCreateTime "
					+ "(DeviceID, RecTime, CreateTime) VALUES (?, ?, ?);";
