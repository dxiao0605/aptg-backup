                        ...
                        <tbody className='tbody'>
                            {tbody?.length > 0 ?
                                tbody.slice(row * (active - 1), row * active)
                                    .map((item, idx) => {
                                        return (
                                            <tr key={item.Seq}>
                                                {
                                                    tableHeader?.length > 0 && tableHeader.map((itemH, idxH) => {
                                                        if (item.Link && itemH.sortName === "BatteryGroupID") {
                                                            return (
                                                                tableHeader[idxH].active && <td key={itemH.id + idx} >
                                                                    ...
                                                                    }}>{item[itemH.sortName]}</Link></td>
                                                            )
                                                        }else {
                                                            return (
                                                                tableHeader[idxH].active && <td key={itemH.id + idx} >{item[itemH.sortName]}</td>
                                                            )
                                                        }
                                                    })
                                                }
                                ...
                                : <tr className="text-center td_btn-config">
                                    <td colSpan={limits.Edit === 1 ? tableHeader.length : tableHeader.length} >{tableErrMsg}</td>
                                </tr>}
                        </tbody>
                    </table>
                </div>
